...
const db_keys_table = firebase.firestore().collection("Keys");
...
export default class Home extends React.Component {
  ...
  componentDidMount = async () => {
    ...
    const authorsName = authors["Name"];
  ...
  onSubmit = () => {
    ...
    if (mode == "Author-Field") {
      console.log("submit author-feild");
  ...
  processAuthorField = async () => {
    ...
    console.log("fnish cal chart daata");
      ...
      Promise.all(promises)
        .then((snapshots) => {
          ...
          Object.values(doc_objs).forEach((doc) => {
            ...
            const fieldNames = data["marc"]["650"];
            const bookName =
  ...
  processKeyword = async () => {
    ...
    Object.keys(field_author).forEach((field) => {
      chartData.push([field, search, 0, 0]);
        ...
        chartEvents={[
          {
            ...
            callback: ({ chartWrapper }) => {
              ...
              if (selection.length === 1) {
                ...
                const { setAuthor, setField } = this.props;
                setAuthor(search);
